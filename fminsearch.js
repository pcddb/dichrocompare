...
fminsearch2=function(fun,Parm0,x,y,Opt){// fun = function(x,Parm)
	// example
	//
	// x = [32,37,42,47,52,57,62,67,72,77,82,87,92];y=[749,1525,1947,2201,2380,2537,2671,2758,2803,2943,3007,2979,2992]
	// fun = function(x,P){return x.map(function(xi){return (P[0]+1/(1/(P[1]*(xi-P[2]))+1/P[3]))})}
	// Parms=jmat.fminsearch2(fun,[100,30,10,5000],x,y)
	//
	// Another test:
	// x=[32,37,42,47,52,57,62,67,72,77,82,87,92];y=[0,34,59,77,99,114,121,133,146,159,165,173,170];
	//
	// Opt is an object will all other parameters, from the objective function (cost function), to the 
	// number of iterations, initial step vector and the display switch, for example
	// Parms=fminsearch2(fun,[100,30,10,5000],x,y,{maxIter:10000,display:false})
	...
	for(var i=0;i<Opt.maxIter;i++){
        ...
        if(P0[5]<P0[2]){
            P0[5]=P0[2]
            step[5]=math.abs(step[5])
            step[2]=-math.abs(step[2])
        }
        //check m3>m2
        if(P0[7]<P0[5]){
            P0[7]=P0[5]
            step[7]=math.abs(step[7])
            step[5]=-math.abs(step[5])
        }
	...
	if (!!document.getElementById('plot')){ // if there is then use it
		fminsearch2.plot(x,y,fun(x,P0),P0);
	}
	return P0
};

fminsearch2.load=function(src){ // script loading
	// example: fminsearch2.load('http://localhost:8888/jmat/jmat.js')
	var s = document.createElement('script');
	s.src = src;
	document.head.appendChild(s);
	s.parentElement.removeChild(s);
...
fminsearch2.plot=function(x,y,yp,Parms){ // ploting results using <script type="text/javascript" src="https://www.google.com/jsapi"></script>
	// create Array in Google's format
	var data = new google.visualization.DataTable();
	data.addColumn('number', 'X');
	data.addColumn('number', 'Observed');
